...
- 在调用 Quarto 前插入列表块边界（不改 vault 源文件），有序列表同样处理。

## 0.1.7

- 新增 QMD 编辑区与实时预览的双向同步滚动，并提供工具栏开关。
- 改进源码行与预览块的锚点映射，过滤不可靠及非单调锚点，提升长文档定位准确度。
- 修复预览重渲染后同步状态可能失效的问题，并避免双向滚动形成反馈循环。
- 修复连续 Markdown 无序列表在 QMD 转换后丢失列表边界、无法正常渲染的问题。
- 补齐实时预览正文背景，避免预览区域出现底色断层。
